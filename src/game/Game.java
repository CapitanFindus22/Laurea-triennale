...
public final class Game extends JPanel {
		...
		new SwingWorker<> () {
			...
			protected Void doInBackground() throws Exception {
				...
				if(player.getStats().getSpeed() > enemy.getStats().getSpeed()) {
					...
					if(Pokemon.doesHit(player,move,enemy)) {
					...
					if(!checkKo()) {
						...
						//Check if the move hit
						if(Pokemon.doesHit(player,enemy,move)) {
	...
	private Move getRandomMove() {
		...
		return enemy.getMoves()[r.nextInt(index)];
	...
	private void checkNewMove() {
		...
		case 3:
			...
			if(!commands.getThirdMove().isEnabled()) commands.getThirdMove().setEnabled(true);
		...
		case 4:
			...
			if(!commands.getFourthMove().isEnabled()) commands.getFourthMove().setEnabled(true);
